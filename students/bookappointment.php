...
private $the_time;
    ...
    private function clashingAppointments($tm,$dt,$cnl){
        ...
        $search_if_exist = "select * from sessions where  date= ? AND 
    ...
    public function bookAppointment($regno,$names,$couns,$dt,$st_tm,$en_time){
        ...
        if($errorinBooking->clashingAppointments($st_tm,$dt,$couns)==true){
            ...
            if($errorinBooking->validateTime($st_tm)==true){
                ...
                $create_Appointment_session = "insert into sessions(studentReg, names ,counsellor, date, st_time, en_time) values
					('$regno','$couns','$dt','$st_tm','$en_time')";
...
if(isset($_POST['book'])){
    ...
    $bookAppointment = new NewAppointment($student_regno,$student_name,$counsellor_picked,$the_date,$the_time,$end_appointment);
