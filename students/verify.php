				...
				if(empty($_GET['id']) && empty($_GET['code']))
				{
					$user->redirect('index.php'); //if ID# and code# are empty disallow access to here and redirect to index page.
														...
														if(isset($_GET['id']) && isset($_GET['code'])) //if ID and code are set, begin processing...
														{
															...
															$stmt = $user->runQuery("SELECT userID,userStatus FROM student WHERE userID=:uID AND tokenCode=:code LIMIT 1");
															...
															if($stmt->rowCount() > 0)
															{
																...
																{
																	$stmt = $user->runQuery("UPDATE student SET userStatus=:status WHERE userID=:uID");
